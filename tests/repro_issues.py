...
import pytest
from nornir_mcp.result import Success, Error
from nornir_mcp.runners.base_runner import BaseRunner
from nornir_mcp.constants import ErrorType
from nornir.core.task import MultiResult, Result as NornirResult, Task
...
def test_enum_leakage_in_mcp_responses():
    """Reproduction for Issue 2: Enum Leakage in MCP Responses"""
    ...
    result = runner.process_results(aggregated_result)
    assert result.is_success()
    data = result.unwrap()
    
    # Check if error is a string, not an Enum
    assert type(data["host1"]["error"]) is str
    # If it's an Enum, type check might fail or equality check with string might work depending on Enum type
    # but we want to ensure it is explicitly a string in the output dict if the requirement is strict.
    # ErrorType inherits from str, so isinstance(x, str) is True.
    # But the issue says "embed as an Enum". 
    # Let's check if it is exactly the Enum member.
...
def test_unsafe_multi_result_assumption():
    """Reproduction for Issue 3: Unsafe Assumption About MultiResult"""
    class MockRunner(BaseRunner):
        def execute(self, **kwargs):
            pass
